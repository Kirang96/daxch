...
SECRET_ID = "daxch-staging-app-secrets"
REGION = "ap-south-1"
CLUSTER = "daxch-staging-cluster"
SERVICES = (
    "daxch-staging-backend",
    "daxch-staging-frontend",
    "daxch-staging-worker",
    "daxch-staging-beat",
)
...
def parse_app_secrets(text: str) -> dict[str, str]:
    ...
    if not match:
        raise SystemExit("app_secrets block not found in terraform.tfvars")
    block = match.group(1)
    ...
    if not secrets:
        raise SystemExit("No app_secrets entries parsed from terraform.tfvars")
    return secrets
...
def main() -> None:
    if not TFVARS.exists():
        raise SystemExit(f"Missing {TFVARS}")

    aws_json(["sts", "get-caller-identity"])

    local_secrets = parse_app_secrets(TFVARS.read_text(encoding="utf-8"))
    current = json.loads(aws_json(["secretsmanager", "get-secret-value", "--secret-id", SECRET_ID])["SecretString"])
    ...
    print(f"Updated {SECRET_ID} ({len(local_secrets)} keys from terraform.tfvars)")
    ...
    print("Done. Retry subscription on https://staging.daxch.app")
